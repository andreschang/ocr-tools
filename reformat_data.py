...
directories = {"cesm-raw": "/Volumes/Samsung_T5/Open_Climate_Research-Projects/data/raw/cesm/", \
...
## Primary functions  ##
########################
def reformat_data(variable, yr0, yrf, dt = 'daily', mode = 'cesm', cesm_mem = 2, cesm_hemisphere = 'nh',
  ...
  var, units, dim, axes, lat0, lon0, level0, cellarea0, data_yr0 = extract_data(variable, \
  ...
  ndiv = ndivs[dt]
  nt, offset = timesync(yr0, yrf, dt = dt, data_yr0 = data_yr0)
  var = var[offset:]

  ## 2. MAKE SIMPLIFIED, SEGMENT FILES
  ##
  summary, sum_tag = reformat_summary(variable, yr0, yrf, dim, ndiv = ndivs[dt])
  ...
  for i in range(int(nt/120)):
    ...
    f_out = outfile(variable, output = output, dt = dt, mode = mode, mem = cesm_mem,
  ...
  summary += '\n\n'+add_to_report
  sum_out = outfile(variable, output = 'summary', dt = dt, mode = mode, mem = cesm_mem,
    base_yr = base_yr, yr0 = yr0, yrf = yrf, custom_tags = sum_tag)

  np.savetxt(sum_out, [summary], fmt = '%s')

def spatial_average(variable, yr0, yrf, dt = 'daily', mode = 'cesm', cesm_mem = 2, cesm_hemisphere = 'nh',
  file = '', data_yr0 = -1,  lat_name = 'lat', lon_name = 'lon', level_name = 'level', cellarea_name = 'tarea',
  dim = ['time', 'lat', 'lon'], lat_bounds = [-89., 89.], lon_bounds = [-179., 179.], saveto = '', custom_tags = [],
  lon_bound_convert = True, output = 'plot'):
  ...
  if grab_reformatted_data(variable, yr0, yrf, dt = dt, mode = mode, cesm_mem = cesm_mem, \
  cesm_hemisphere = cesm_hemisphere):
    target_ncs, start_index, end_index  = grab_reformatted_data(variable, yr0, yrf, dt = dt, \
  ...
  for i in range(nsubmit):
    ...
    lat_indices, lon_indices = get_latlon_indices(lat0, lon0, lat_bounds, lon_bounds, print_summary = print_summary)

    if len(lat0.shape) == 1:
      bounded_var = bound_var(var, lat_indices, lon_indices, lat_axis = axes['lat'], lon_axis = axes['lon'])
      mm_var = np.mean(bounded_var, axis = axes['lon'])

      lat_range = [lat0[k] for k in lat_indices]
      wgt = reg_wgt(lat_range[0], lat_range[-1], len(lat_range))
      mvar0 = 0
  ...
  if output == 'plot':
    ...
    plt.plot(xtime, mvar)
    f1 = outfile(variable, output = 'spatial_average', dt = dt, mode = mode, mem = cesm_mem, yr0 = yr0,\
...
###################################
def extract_data(variable, yr0, yrf, dt = 'daily', mode = 'cesm', cesm_mem = 2, cesm_hemisphere = 'nh',
  ...
  if mode == 'cesm':
    ...
    file, data_yr0, var_name, lat_name, lon_name = grab_cesm_data(variable, yr0, yrf, \
      mem = cesm_mem, dt = dt, hemisphere = cesm_hemisphere, mode = 'reformat_data')
...
def grab_cesm_data(variable, yr0, yrf, mem = 2, dt = 'daily', hemisphere = 'nh',
  data_folder = directories['cesm-raw'], subfolders = ['dt', 'variable'], mode = 'getfile'):
  ...
  if mode == 'reformat_data':
    return f, data_yr0, var_name, lat_name, lon_name
  elif mode == 'getfile':
    return f

def grab_reformatted_data(variable, yr0, yrf, dt = 'daily', mode = 'cesm', cesm_mem = 'any',
  cesm_hemisphere = 'nh', print_summary = False):

  cesm_mem = 2

  if mode == 'cesm':
    f0 = directories["cesm-reformatted"]+variable+'/'
  else:
    f0 = directories["other-reformatted"]+variable+'/'
...
def get_latlon_indices(lat, lon, lat_bounds = [-89., 89.], lon_bounds = [-179., 179.], \
  ...
  if(print_summary == True):
    ...
    f1 = directories['scratch']+'bounded_area.'+scratchId+'.png'
...
#####################################
def timesync(yr0, yrf, dt = 'daily', data_yr0 = -1):
  ndiv = ndivs[dt]
  offset = (yr0-data_yr0)*ndiv
  nt = (yrf-yr0+1)*ndiv
  return nt, offset

def bound_var(var, lat_indices, lon_indices, lat_axis = 1, lon_axis = 2):
  ## trims variable down
...
def outfile(variable, output = 'sequence', dt = 'daily', mode = 'custom', mem = -1, 
  ...
  exts = {'sequence': 'nc', 'date': 'nc', 'summary': 'txt', 'spatial_average': 'png'}
  f1 = [variable, abbrev[dt]]
  if mode == 'cesm':
    f0 = directories["cesm-reformatted"]
    f1.append('m'+'{:03d}'.format(mem))
  else:
    f0 = directories["other-reformatted"]
  ...
  elif output == 'spatial_average':
    f0 = directories["scratch"]
...
def name_sync(source_file, keyword, replace):
  ...
  for file in os.listdir(parent_folder):
    if ((scratchId in file) and (replace in file)):
      target_file = parent_folder+'/'+file
      print(target_file)
  new_name = source_file.replace(keyword, replace)
